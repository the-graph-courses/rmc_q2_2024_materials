---
title: "Household survey datasets from Cameroon"
...
EDIT/CLARIFY/image? The surveyor goes to the same house 7 days after the initial visit, then 21 days after, to collect individual level data for each child present.

## Phase 2: Visit on Day 7

Read in the file "child_data_day_07.csv" from the data folder.
...
Q1B: Repeat the steps from Question 1A, but this time add household variables to the phase 3 data frame, `child_data_day_21`.

```{r}
merged_d21 <- "WRITE_YOUR_CODE_HERE"
```

Look through the records in these two datasets. You may notice that some children have all NAs for household variables, such as `type_residence`. This indicates that these children were unhoused, and surveryed on the street. We will investigate this further in later questions.

## Question 2: Identify missing records

Q2A: Using the `merged_d07` and `merged_d21` datasets you created above, identify children who were surveyed in Phase 2 (Day 7) but not in Phase 3 (Day 21).
...
Q2B: Similarly, do the opposite comparison to identify children who were surveyed on Day 21 but were not originally surveyed on Day 21.
...
Q3A) Create a bar plot of residence type for children in `only_d07`. You should have 3 bars: suburban, urban, and NA. The NA records correspond to houseless children. You plots should demonstrate that more houseless children found during the last phase (Day 21) compared to Day 7.

HINT: Use geom_col() from ggplot2.

```{r}
"WRITE_YOUR_CODE_HERE"
```

Q3A) Create the same bar plot of residence type for children in `only_d21`.
...
Q4A) Identify the correct joining function to give a dataset with records of children who were surveryed on BOTH days.

```{r}
both_days <- "WRITE_YOUR_CODE_HERE"
```

Q4B) Compare a health variable such as `deworming` for day 7 vs day 21. Present a summary table showing this information.
